...
.. code-block:: python
    ...
    import asyncio
    from core.dlq import setup_dlq

    async def main():
        # Initialize DLQ with Redis URL
        dlq_manager = await setup_dlq("redis://localhost:6379")

        # Custom retry handler
        async def retry_handler(task: FailedTask):
            # Implement your retry logic here
            pass

        # Update retry handler
        dlq_manager.retry_handler = retry_handler
...
.. code-block:: python
    ...
    from core.dlq import FailedTask, FailureReason

    async def handle_processing_error(task_id: str, video_path: str, error: Exception):
